	...
	case 4:
		{
			...
			for(int i = 0; i < j; i++) 
			{
				if(std[i].roll_no == Roll_No)
				{
					...
					/*std[j] = std[i];*/					// This is shallow copying, matlab ye ke just reference copy hota hai
					cout << "Enter New Roll Number : ";	// kisi b stucture variable ko dusre structure variable ke equal krne se,
					cin >> Roll_No;						// so explicitly har member ko assign karo theek, koi problem nahi hogi,
					std[j].roll_no = Roll_No;			// even though this program works lekin jab records agay peeche honge to
					cout<<"Roll no. : "<<std[j].roll_no<<endl;
					cout<<"Name: "<<std[j].name<<endl;
					cout<<"City: "<<std[j].city<<endl;
					cout<<"Phone: "<<std[j].phone<<endl;
